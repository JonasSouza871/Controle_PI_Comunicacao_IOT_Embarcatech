...
void ssd1306_init(ssd1306_t *ssd, uint8_t width, uint8_t height, bool external_vcc, uint8_t address, i2c_inst_t *i2c) {
    ...
    if (ssd->ram_buffer != NULL) {
        ssd->ram_buffer[0] = 0x40; //Define prefixo de dados (Co=0, D/C=1)
    }
    ssd->port_buffer[0] = 0x80; //Define prefixo de comando
}
...
void ssd1306_pixel(ssd1306_t *ssd, uint8_t x, uint8_t y, bool value) {
    uint16_t index = (y / 8) * ssd->width + x + 1; //Calcula posição no buffer
    uint8_t pixel = y % 8; //Bit correspondente no byte
    if (value) {
...
//Desenha números pequenos (5x5 pixels) usando a fonte
void ssd1306_draw_small_number(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
    if (c >= '0' && c <= '9') {
        uint16_t index = ((c - '0') * 5) + (69 * 8); //Offset da fonte para números
...
//Desenha um caractere usando a fonte
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y, bool use_small_numbers) {
    ...
    //Mapeia caracteres para índices na fonte
    if (c >= '0' && c <= '9') {
        index = (c - '0' + 1) * 8;
    } else if (c >= 'A' && c <= 'Z') {
        index = (c - 'A' + 11) * 8;
        ...
        index = 69 * 8; //Símbolo de exclamação
        rotate = true;
    }
...
void ssd1306_draw_string(ssd1306_t *ssd, const char *str, uint8_t x, uint8_t y, bool use_small_numbers) {
    while (*str) {
        ...
        //Verifica quebra de linha automática
...
void ssd1306_rect(ssd1306_t *ssd, uint8_t top, uint8_t left, uint8_t width, uint8_t height, bool value, bool fill) {
    ...
    //Preenche o interior, se solicitado
    if (fill) {
